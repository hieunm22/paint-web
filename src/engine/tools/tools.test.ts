...
describe("PencilTool", () => {
	...
	it("paints color 2 when the gesture began on the right button", () => {
...
describe("BrushTool", () => {
	it("strokes with color 1, and with color 2 on the right button", () => {
		const left = harness()
		const right = harness()
...
describe("EraserTool", () => {
	it("paints color 2 rather than cutting a hole", () => {
		const h = harness()
	...
	it("replaces only color 1 on the right button", () => {
		const h = harness(3)
		h.pixels.set([0, 0, 0, 255], (1 * DOC + 1) * 4) // one color 1 pixel
...
describe("ShapeTool colors", () => {
	...
	it("strokes the outline in color 1", () => {
		expect(draw({})).toEqual([{ kind: "stroke", style: RED }])
	})

	it("fills the interior with color 2 once a fill style is picked", () => {
