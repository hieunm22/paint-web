...
/**
 * hue, saturation and luminance on the scale the Windows color dialog shows:
...
export interface Modifiers {
	/** right button, or Ctrl+click on macOS: the gesture uses color 2. */
	secondary: boolean
	shift: boolean
	alt: boolean
...
export interface ToolContext {
	...
	outline: StrokeStyle
	fill: StrokeStyle
	/** color 2 drops out of a lifted selection when this is on. */
	transparent: boolean
...
export interface FillResponse {
	buffer: ArrayBuffer
	/** the box that changed, or null when the seed already held that color. */
	dirty: Rect | null
}
...
export interface Tool {
	...
	/** redraws the held object after a color, size or style change. */
...
/** one cubic or straight step of a normalized shape outline. */
...
export interface ShapeDraft {
	kind: ShapeKind
	/** the right button swaps the outline and fill colors. */
	swapped: boolean
	box: Rect
